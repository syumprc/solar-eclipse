...
/* If input not from terminal, exit here and let script sort re-invoke */

  if (!isatty(STDIN_FILENO))
      exit (EXIT_FAILURE);

/* Original version did execvp, replacing old with new image */
...
cleanup_rlwrap_and_exit(int status)
{
  ...
  if (status != EXIT_SUCCESS)  /* rlwrap itself has failed, rather than the wrapped command */
	...
	exit (EXIT_SUCCESS); /* Don't propagate exit status from command */
