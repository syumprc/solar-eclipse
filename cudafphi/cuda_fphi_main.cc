...
void write_to_hdf5(string out_filename, string header_filename, float * h2r[] , float * indicator[] , float * pvals [] , size_t n_voxels, bool use_pval){
	string h2r_filename = out_filename + "_h2r_connectivity_matrix.h5";
	string indicator_filename = out_filename + "_indicator_connectivity_matrix.h5";
	...
	string labels[n_voxels];

	while(header_in >> header && current_voxel < n_voxels){
		labels[current_voxel] = header.c_str();
		current_voxel++;
	}
	header_in.close();

	write_data_to_hdf5(h2r_filename.c_str(), labels, h2r, n_voxels);

	write_data_to_hdf5(indicator_filename.c_str(), labels, indicator, n_voxels);

	if(use_pval){
		string pval_filename = out_filename + "_pvalues_connectivity_matrix.h5";
		write_data_to_hdf5(pval_filename.c_str(), labels, pvals, n_voxels);
	}
...
int main (int argc, const char *argv[]){
	...
	try{
 	...
 	string h2r_filename = file_out + "_h2r_connectivity_matrix.csv";
 	string indicator_filename = file_out + "_indicator_connectivity_matrix.csv";
 	string pvalue_filename = file_out + "_pvalue_connectivity_matrix.csv";
	if(use_qsub == true && node_number == 0){
		write_column_line(h2r_filename, header_filename, n_voxels);
		write_column_line(indicator_filename, header_filename, n_voxels);
		if(get_pval)
			write_column_line(pvalue_filename, header_filename, n_voxels);
	}
	...
	if(use_qsub){
 	...
 	float * pvals;
 	float * h2;
 	float  * conn_h2[qsub_batch_size];
 	float * conn_indicator[qsub_batch_size];
 	float * conn_pvalues[qsub_batch_size];


 	if(get_connectivity){


 		for(size_t voxel = 0; voxel < qsub_batch_size ; voxel++){

 	 		conn_h2[voxel] = new float[n_voxels];

 	 		conn_indicator[voxel] = new float[n_voxels];
 	 		if(get_pval)
 	 			conn_pvalues[voxel] = new float[n_voxels];
 	...
 	string headers[qsub_batch_size];
 	if(use_qsub){



 		call_cudafphi_qsub( conn_h2, conn_indicator,
 		...
 		get_headers(header_filename, headers,  node_number,  n_voxels/total_nodes,  qsub_batch_size);


 		write_row_data(h2r_filename, headers ,  conn_h2, qsub_batch_size, n_voxels);
 		write_row_data(indicator_filename, headers ,  conn_indicator, qsub_batch_size, n_voxels);

 		if(get_pval)
 			write_row_data(pvalue_filename, headers ,  conn_pvalues, qsub_batch_size, n_voxels);
 	 	...
 	 	call_cudafphi(h2, indicator, pvals, conn_h2, conn_indicator, conn_pvalues, h_y, aux, cov, evectors, pmatrix, covariates, get_pval, get_connectivity, n_voxels, n_subjects, n_permutations, n_covariates, selected_devices);
 		if(get_connectivity){
 			...
 			if(get_pval)
 	 		...
 	 		get_headers(header_filename, headers,  0, n_voxels,  n_voxels);

 	 		write_row_data(h2r_filename, headers ,  conn_h2, n_voxels, n_voxels);
 	 		write_row_data(indicator_filename, headers ,  conn_indicator, n_voxels, n_voxels);

 	 		if(get_pval)
 	 			write_row_data(pvalue_filename, headers ,  conn_pvalues, n_voxels, n_voxels);


 		//	write_to_hdf5(file_out, header_filename,  conn_h2, conn_indicator , conn_pvalues ,  n_voxels,  get_pval);
 	...
 	if(get_connectivity == false){
 		...
 		for(size_t col = 0 ; col < qsub_batch_size ; col++){
 			delete [] conn_h2[col];
 			delete [] conn_indicator[col];

 			if(get_pval)
 				delete [] conn_pvalues[col];
 		}
